        ...
        try:
            with open(csv_path, "r", encoding="utf-8") as csvfile:
                ...
                with open(txt_path, "w", encoding="utf-8") as txtfile:
                                ...
                                try:
                                    ...
                                    try:
                                        ...
                                        if row_date.tzinfo is not None:
                                            row_date = row_date.astimezone().replace(
                                                tzinfo=None
                                            )
                                except Exception as e:
                                    print(
                                        f"无法解析 {csv_file} 中的日期 {row.get('updated_at')}: {e}"
                                    )
                                    continue
                    ...
                    print(
                        f"总计 {total_count} 个角色，筛选后保留 {filtered_count} 个角色"
                    )
        except Exception as e:
...
def interactive_cli():
    print("欢迎使用CSV转TXT工具")
    print(
        "本工具将把outputs文件夹中的CSV文件转换为TXT文件，并根据post数量及updated_at日期筛选角色"
    )
    ...
    try:
        ...
        if updated_at_threshold:
            print(
                f"日期过滤阈值设置为: {updated_at_threshold.strftime('%Y-%m-%d %H:%M:%S')}"
            )
