...
tag_2_search = "dark-skinned_female "
...
remaining_tags = [
    tag for tag in tag_related.get("related_tags") if tag not in female_matched_tags
]
for keyword in male_gender_keywords:
    gender_tags = [
        tag for tag in remaining_tags if keyword in tag.get("tag")["name"].lower()
    ]
    if gender_tags:
        print(f"\n包含 '{keyword}' 的标签:")
        for tag in gender_tags:
